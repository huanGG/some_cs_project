...
{
    /* Fill in */
    struct bisector *p = malloc(sizeof(struct bisector));
...
char *getBisectorEquation(struct bisector *b)
{
    ...
    if (b->a == 0)
    {
        ...
        int stringLength = snprintf(returnString, 100, "x = %lf",
                                    b->b);
        // returnString = (char *) malloc(sizeof(char) * (stringLength + 1));
        assert(returnString);
        sprintf(returnString, "x = %lf", b->b);
    }
    else
    {
        /* Find out memory needed. */
        int stringLength = snprintf(returnString, 100,
                                    "y = %lf * (x - %lf) + %lf", b->k, b->b,
                                    b->c);
        // returnString = (char *) malloc(sizeof(char) * (stringLength + 1));
        assert(returnString);
...
char *getIntersectionString(struct intersection *intersection)
{
    ...
    int start_edge = intersection->start_edge;
    int end_edge = intersection->end_edge;
    if (start_edge <= end_edge)
    {
        /* Find out memory needed. */
        int stringLength = snprintf(returnString, 0,
                                    "From Edge %d (%lf, %lf) to Edge %d (%lf, %lf)",
                                    start_edge, intersection->start_x, intersection->start_y,
                                    end_edge, intersection->end_x, intersection->end_y);
        // returnString = (char *) malloc(sizeof(char) * (stringLength + 1));
        assert(returnString);
...
struct intersection *getIntersection(struct bisector *b, struct DCEL *dcel, int face,
                                     double minLength)
{
    struct face f = dcel->faces[face];
    struct halfEdge *start_he = f.he;
    struct halfEdge *curr_he = start_he;
    double x, y;
    int is_start = 1;
    int find_edge = 1;
    struct intersection *is = malloc(sizeof(struct intersection));
    while (curr_he != start_he || is_start == 1)
    {
        is_start = 0;
        if (intersects(curr_he, b, dcel, minLength, &x, &y) != DOESNT_INTERSECT)
        {
            if (find_edge == 1)
            {
                is->start_edge = curr_he->edge;
                is->start_x = x;
                is->start_y = y;
                find_edge = 2;
            }
